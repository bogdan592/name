init -2 python:
    ...
    class Enemy:
        def __init__(self,x,y,nemesis, active=False):
            self.x = x
            self.y = y
            self.active = false
            self.nemesis = nemesis
            if player.won == True:
                self.x,self.y = -100000000.0,-100000000.0
    ...
    class Trigger:
        def __init__(self,x,y,target,shroom, active=True):
            self.x = x
            self.y = y
            self.target = target
            self.shroom = shroom
    ...
    class Banner:
        ...
        def activate(self):
            ...
            self.Enemy_x,self.Enemy_y = -1.0,-1.0
    
    player=Player(0.5,0.5,0.05,0.05,0.95,0.95)
    winning_banner = Banner(0.5,0.5, player)
    shroom = Enemy(0.2,0.2,player,active=False)
    exit_trigger = Trigger(0.9,0.1, winning_banner,shroom, active=False)
    control_trigger = Trigger(0.8,0.8,exit_trigger)
    #shroom = Enemy(0.2,0.2,player,active=True)
...
screen hamster_cage:
    ...
    add "bg field" anchor (0.5,0.3)  at Transform(function=main_arena.transform)
